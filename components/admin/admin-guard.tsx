...
export function AdminGuard({ children }: { children: React.ReactNode }) {
  ...
  if (status === 'loading' || roles.isLoading) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#f5f5f7] px-4 text-[#1d1d1f]">
        <div className="flex items-center gap-3.5 rounded-2xl bg-white px-6 py-5 shadow-sm border border-black/[0.08]">
  ...
  if (status === 'unauthenticated' || isGuest) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#f5f5f7] px-4 text-[#1d1d1f]">
        <div className="max-w-md w-full rounded-3xl bg-white p-8 text-center shadow-sm border border-black/[0.08]">
          <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-black/[0.04] text-[#1d1d1f] border border-black/[0.06]">
          ...
          <div className="mt-6 flex flex-col gap-2.5">
            <Link
              ...
              className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-[#1d1d1f] py-3 text-xs sm:text-sm font-semibold text-white shadow-xs hover:bg-black transition"
            >
              <LogIn className="h-4 w-4" /> Sign In as Superadmin
            </Link>
            <Link
              href={'/home' as any}
              className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-white border border-black/10 py-3 text-xs sm:text-sm font-medium text-[#1d1d1f] hover:bg-black/[0.03] transition shadow-xs"
            >
  ...
  if (!isAuthorized) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-[#f5f5f7] px-4 text-[#1d1d1f]">
        <div className="max-w-lg w-full rounded-3xl bg-white p-8 text-center shadow-sm border border-black/[0.08]">
          <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-red-50 text-red-600 border border-red-200/60">
            <ShieldAlert className="h-6 w-6" />
          </div>
          <span className="mt-4 inline-block rounded-full bg-red-50 px-2.5 py-0.5 text-[11px] font-semibold text-red-700 border border-red-200/60">
            403 Forbidden
          </span>
          ...
          <div className="mt-6 flex flex-col sm:flex-row gap-2.5">
            <Link
              href={'/home' as any}
              className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-xl bg-white border border-black/10 py-2.5 text-xs font-semibold text-[#1d1d1f] hover:bg-black/[0.03] transition shadow-xs"
            >
