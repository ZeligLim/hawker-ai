  ...
  return (
    <>
      <div className="fixed bottom-24 right-4 z-50 flex flex-col items-center gap-2">
        {isListening && (
          <div className="absolute -top-12 px-3 py-1.5 bg-black text-white text-xs font-semibold rounded-full shadow-lg animate-pulse whitespace-nowrap">
            Listening...
          </div>
        )}
        ...
        <button
          ...
          onClick={handleClick}
          className={`flex h-14 w-14 items-center justify-center rounded-full text-white shadow-lg transition-all transform ${isListening ? 'bg-red-500 scale-110 shadow-red-500/50' : 'bg-black hover:bg-neutral-800 hover:scale-105'}`}
        >
          {isListening ? (
            ...
            <form onSubmit={onSubmit} className="flex gap-2">
              <div className="relative flex-1">
                <input
                  ...
                  className="w-full h-11 pl-4 pr-10 bg-neutral-100 rounded-full text-sm outline-none focus:bg-neutral-200 transition-colors"
                />
                <button
                  type="button"
                  onClick={startListening}
                  className="absolute right-2 top-1/2 -translate-y-1/2 h-7 w-7 flex items-center justify-center text-neutral-500 hover:text-black transition-colors"
                >
              ...
              <button 
                type="submit"
                disabled={!input.trim()}
                className="h-11 w-11 flex items-center justify-center bg-black text-white rounded-full shrink-0 disabled:opacity-50 disabled:bg-neutral-300 transition-colors"
              >
