    ...
    <div className="fixed inset-0 w-screen h-screen overflow-hidden bg-white text-black">
      ...
      <div className="fixed top-3.5 inset-x-3.5 sm:inset-x-4 max-w-lg mx-auto z-40 flex flex-col gap-2 pointer-events-auto">
        ...
        <div className="flex items-center justify-between gap-2">
          {/* Map vs List View Segmented Pill (Standard Apple touch target, pure white & black, 0 border) */}
          <div className="flex items-center bg-white p-1 rounded-full shadow-md">
            <button
              type="button"
              onClick={() => setViewMode('map')}
              className={`flex h-9 items-center gap-1.5 rounded-full px-4 text-xs font-semibold transition-colors ${
                viewMode === 'map'
          ...
          <div className="flex items-center gap-2">
            {/* Quick Aircon Filter Pill (Pure White inactive, Pure Black active, h-11, 0 border) */}
            <button
              type="button"
              onClick={() => setAirconOnly((prev) => !prev)}
              className={`flex h-11 items-center gap-2 rounded-full px-4 text-xs font-semibold shadow-md transition-colors ${
                airconOnly
                  ? 'bg-black text-white'
            ...
            {/* Quick Scan QR Button (Standard 44px h-11, 0 border) */}
            <Link
              href="/scan"
              title="Scan QR"
