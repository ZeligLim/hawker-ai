...
function OperatingScheduleDialogContent({
 ...
 return (
 <div className="fixed inset-0 z-50 flex flex-col bg-[#f5f5f7] slide-in-from-bottom-4 " role="dialog" aria-modal="true">
 {/* Header */}
 <div className="flex shrink-0 items-center .04] bg-[#f5f5f7] px-4 py-3 sm:px-6 gap-3">
 <button type="button" onClick={onClose} className="flex h-10 w-10 items-center justify-center rounded-full bg-white text-[#1d1d1f] shadow-xs hover:bg-black/5 -ml-1 shrink-0" aria-label="Go back">
 ...
 />
 <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after: after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after: after: after:rounded-full after:h-5 after:w-5 after: peer-checked:bg-blue-600"></div>
 </label>
 </div>
 ...
 className="inline-flex h-11 sm:h-8 w-full sm:w-auto items-center justify-center gap-1.5 rounded-full bg-black px-4 text-xs font-semibold text-white hover:bg-neutral-800 shadow-xs">
 ...
 <div
 key={key}
 className={`rounded-2xl p-4 shadow-xs ${day.isOpen ? 'bg-white' : 'bg-white/50'}`}
 >
 ...
 className="rounded text-blue-600 focus: h-4 w-4"
 />
 <label
 htmlFor={`day-${key}`}
 ...
 className="px-3 py-2 text-xs rounded-[10px] bg-[#f5f5f7] text-[#1d1d1f] font-mono font-medium outline-none focus:bg-[#e5e5ea] "
 />
 ...
 {/* Footer Actions */}
 <div className="flex items-center justify-start gap-2.5 px-6 py-4 bg-[#f5f5f7] .04] sticky bottom-0">
 <SaveCancelButtons
 type="submit"
 isSaving={isSaving}
 onCancel={onClose}
