...
function AuthForm() {
 ...
 disabled={isGoogleLoading || isSubmitting}
 className="mt-3 flex w-full items-center justify-center rounded-full ] bg-[#f5f5f7] px-4 py-3 text-sm font-medium text-[#1d1d1f] disabled:cursor-not-allowed disabled:opacity-60"
 >
 Continue as guest
 </button>
 )}
 ...
 <div className="flex items-center gap-2 rounded-[18px] bg-[#f5f5f7] px-3 py-3 focus-within:]">
 ...
 {error ? (
 <div className="rounded-[18px] ] bg-[#fff1f2] px-3 py-2 text-sm text-[#9f1239]">
 {error}
 </div>
 ) : null}

 {success ? (
 <div className="rounded-[18px] ] bg-[#ecfdf5] px-3 py-2 text-sm text-[#166534]">
 {success}
 </div>
 ) : null}
 ...
 className="flex w-full items-center justify-center rounded-full bg-[#111827] px-4 py-3 text-sm font-medium text-white shadow-[0_12px_24px_rgba(17,24,39,0.18)] disabled:cursor-not-allowed disabled:opacity-60"
 >
 {isSubmitting ? <LoaderCircle className="h-4 w-4 " /> : mode === 'signin' ? 'Sign In' : 'Sign Up'}
 </button>
 </form>
...
export default function AuthPage() {
 ...
 <LoaderCircle className="mx-auto h-8 w-8 text-[#1d1d1f]" />
