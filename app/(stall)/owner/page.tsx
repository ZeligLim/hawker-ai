...
export default function StallOverviewPage() {
 ...
 {isMasterInactive && (
 <div className="mt-4 flex items-start gap-3 rounded-2xl bg-red-50 p-4 text-red-800 ">
 ...
 {error && (
 <div className="mt-4 flex items-center gap-2 rounded-2xl bg-amber-50 p-3 text-xs text-amber-800 ">
 ...
 {isLoading ? (
 <span className="inline-block h-8 w-20 rounded bg-black/5" />
 ) : (
 stat.value
 )}
 </p>
 ...
 {[1, 2, 3].map((i) => (
 <div key={i} className="h-12 w-full rounded-[16px] bg-[#f5f5f7]" />
 ))}
 </div>
 ...
 key={order.id}
 href="/owner/orders"
 className="flex items-center justify-between gap-3 rounded-[16px] bg-[#f5f5f7] p-3 hover:bg-[#ededf0] "
 >
 ...
 <span
 className={`inline-flex items-center gap-1.5 rounded-full px-2 py-0.5 text-[10px] font-semibold ${statusBadgeColor} shrink-0`}
 >
 ...
 >
 <RefreshCw className={`h-6 w-6 ${isRefreshing ? ' text-amber-600' : ''}`} />
 </button>
 </main>
 );
}
