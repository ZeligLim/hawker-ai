    ...
    <main className="min-h-screen bg-[#f5f5f7] px-4 pb-32 pt-5 text-[#1d1d1f] sm:px-6">
      <div className="mx-auto w-full max-w-md sm:max-w-xl md:max-w-3xl">
        <header className="mb-6 flex items-center gap-3">
          <Link
            href="/owner/menu"
            className="flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-xs hover:bg-neutral-100 transition-colors"
          >
        ...
        {error && (
          <div className="mb-6 rounded-[18px] border border-[#fecaca] bg-[#fff1f2] p-4 text-sm text-[#9f1239]">
            {error}
          </div>
        )}
        ...
        {!hasScanned ? (
          <div className="space-y-6">
            <div 
              ...
              className="relative flex aspect-[4/3] w-full cursor-pointer flex-col items-center justify-center overflow-hidden rounded-[26px] border-2 border-dashed border-neutral-300 bg-white hover:bg-neutral-50 transition-colors"
            >
              {imagePreview ? (
            ...
            {imagePreview && (
              <button
                ...
                onClick={scanMenu}
                disabled={isScanning}
                className="flex h-12 w-full items-center justify-center gap-2 rounded-full bg-black text-sm font-bold text-white shadow-md hover:bg-neutral-800 disabled:opacity-50 transition-colors"
              >
                {isScanning ? (
                  <>
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              {scannedDishes.map((dish, idx) => (
                <div 
                  ...
                  className={`flex cursor-pointer items-start gap-3 rounded-[22px] bg-white p-4 shadow-sm transition-colors border-2 ${
                    dish.selected ? 'border-black' : 'border-transparent hover:border-neutral-200'
                  }`}
                >
                  <div className={`mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border ${
                    dish.selected ? 'border-black bg-black text-white' : 'border-neutral-300 bg-transparent'
                  }`}>
