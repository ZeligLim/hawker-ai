...
const regex = /<div className="mt-4 rounded-\[20px\] bg-\[#f5f5f7\] p-3\.5 \/\[0\.04\]">\s*<div className="flex items-center gap-2">([\s\S]*?)<\/button>\s*<\/div>\s*<\/div>/g;

content = content.replace(regex, (match, innerContent) => {
  // We need to carefully replace the inner content classes
  let newInner = match
